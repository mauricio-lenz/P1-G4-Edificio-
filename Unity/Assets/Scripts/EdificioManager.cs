...
using UnityEngine;
using TipoDiagrama = DiagramaRenderer.TipoDiagrama;
    ...
    public class EdificioManager : MonoBehaviour
    {
        ...
        void ConstruirApoyos()
        {
            foreach (var ap in Raiz.apoyos)
            {
                ...
                txt.text = ap.nodeTag.ToString();
                txt.characterSize = 0.3f;
                txt.fontSize = 40;
                txt.transform.position = ap.Pos + Vector3.up * 1.2f;
            }
        }
        ...
        void ConstruirAreasYCargas()
        {
            foreach (var at in Raiz.areas_tributarias)
            {
                ...
                if (at.poligono != null && at.poligono.Length > 2)
                {
                    ...
                    var txt = go.AddComponent<TextMesh>();
                    txt.text = $"{at.area_trib_m2:0.0} m²";
                    txt.characterSize = 0.5f;
                    txt.fontSize = 36;
                    txt.transform.position = pts[pts.Count / 2] + Vector3.up * 0.6f;
                ...
                if (el != null)
                {
                    ...
                    var txt = go.AddComponent<TextMesh>();
                    txt.text = $"wG={at.wG_kN_m:0.#} wQ={at.wQ_kN_m:0.#} kN/m";
                    txt.characterSize = 0.35f;
                    txt.fontSize = 34;
                    txt.transform.position = mid - Vector3.up *
